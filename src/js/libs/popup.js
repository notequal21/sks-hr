...
class Popup {
  ...
  eventsPopup() {
    ...
    document.addEventListener(
      ...
      function (e) {
        ...
        if (buttonOpen) {
          e.preventDefault();
          let dataValueTemp = buttonOpen.getAttribute(
            this.options.attributeOpenButton
          )
            ? buttonOpen.getAttribute(this.options.attributeOpenButton)
            : 'error';
          dataValueTemp =
            dataValueTemp.indexOf('#') < 0 && dataValueTemp.indexOf('.') < 0
              ? `#${dataValueTemp}`
              : dataValueTemp;
  ...
  _openToHash() {
    ...
    let el = document.querySelector(`${classInHash}`);
    let condition = !el.hasAttribute('data-nohash');
    if (buttons && classInHash && condition) {
      this.open(classInHash);
    }
  }
  // Утсановка хэша
...
document.addEventListener('afterPopupClose', (e) => {
  const { triggerButton, targetOpen } = e.detail.popup;

  if (
    triggerButton.hasAttribute('data-id') &&
    targetOpen.element.querySelector('#vacancies-id')
  ) {
    targetOpen.element.querySelector('#vacancies-id').value =
      triggerButton.hasAttribute('data-id');
  }
});
